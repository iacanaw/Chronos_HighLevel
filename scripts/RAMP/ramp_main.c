...
int main(){
    ...
    //==============PARSER==========================
    ifstream tempFile, powerFile;
    string line;
    int num_lines_temp = 0;
    int num_lines_power = 0;
    int num_samples;

    tempFile.open("temp.log");
    if (!tempFile) {
        cout << "Unable to open tempFile";
        exit(1); // terminate with error
    }

    powerFile.open("power.log");
    if (!powerFile) {
        cout << "Unable to open powerFile";
        exit(1); // terminate with error
    }
    ...
    num_samples = num_lines_power > num_lines_temp ? num_lines_temp : num_lines_power;
    ...
    tempFile.open("temp.log");
    getline (tempFile,line); // remove the header
    while ( getline (tempFile,line) ){
        string line_name;
    ...
    line_count=0;
    powerFile.open("power.log");
    getline (powerFile,line); // remove the header
    while ( getline (powerFile,line) ){
        string line_name;
        stringstream lineStream(line);
        lineStream >> line_name;
